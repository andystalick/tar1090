...
if (SiteName == 'adsbexchange.com tar1090') adsbexchange = true;
...
function setupPlane(hex, plane) {
  ...
  if (hex[0] === '~') {
    ...
    $(plane.tr).style.fontStyle = 'italic';
  } else {
  ...
  if (ShowFlags && plane.icaorange.flag_image !== null) {
    $('img', plane.tr.cells[1]).setAttribute(
      'src',
      FlagPath + plane.icaorange.flag_image
    );
    $('img', plane.tr.cells[1]).setAttribute('title', plane.icaorange.country);
  } else {
    $('img', plane.tr.cells[1]).style.display = 'none';
  }
...
function fetchData() {
  ...
  if (enable_uat) {
    FetchPendingUAT = fetch('chunks/978.json');

    FetchPendingUAT.then(function(data) {
      uat_data = data;
  ...
  var ac_url = [];
  if (adsbexchange) {
    $('#adsbexchange_header').style.display = 'block';
  }
  if (globeIndex) {
    var indexes = globeIndexes();
    var count = 0;
    indexes.sort(function(x, y) {
      if (!globeIndexNow[x] && !globeIndexNow[y]) return 0;
    ...
    for (var i in indexes) {
      ac_url.push(
        'data/globe_' + indexes[i].toString().padStart(4, '0') + '.json'
      );
    }
  } else {
    ...
    req.then(function(data) {
      ...
      if (globeIndex) {
        ...
        if (
          ...
          if (data.west < data.east) {
            var tile = new ol.geom.LineString([
              southWest,
              southEast,
              northEast,
              northWest,
            ...
            var west = new ol.geom.LineString([
              south180p,
              southWest,
              northWest,
              north180p
            ]);
            var east = new ol.geom.LineString([
              south180m,
              southEast,
              northEast,
              north180m
      ...
      if (last == now && !globeIndex) {
        ...
        if (StaleReceiverCount > 5) {
          $('#update_error_detail').textContent =
        ...
        $('#update_error').style.display = 'none';
      }
    });

    req.fail(function(jqxhr, status, error) {
      $('#update_error_detail').textContent =
        'AJAX call failed (' + status + (error ? ': ' + error : '') + ').';
...
function initialize() {
  ...
  try {
    ...
    if (outlineColor != undefined) {
      if (markers.length == 6) OutlineADSBColor = '#' + outlineColor;
    ...
    if (search.has('largeMode')) {
      ...
      console.log(tmp);
      if (!isNaN(tmp)) largeMode = tmp;
    }

    if (search.has('mobile')) onMobile = true;
    if (search.has('desktop')) onMobile = false;

    if (search.has('hideSidebar')) localStorage['sidebar_visible'] = 'false';
    ...
    if (localStorage['SiteLat'] != null && localStorage['SiteLon'] != null) {
      if (
        ...
        SiteLat = CenterLat = DefaultCenterLat = parseFloat(
          localStorage['SiteLat']
        );
        SiteLon = CenterLon = DefaultCenterLon = parseFloat(
          localStorage['SiteLon']
        );
        SiteOverride = true;
      }
    }
  ...
  if (onMobile) enableMouseover = false;
  ...
  if (localStorage['debug'] == 'true') debug = true;
  if (localStorage['debugPosFilter'] == 'true') debugPosFilter = true;
  ...
  configureReceiver.then(function() {
    ...
    fetch(`${databaseFolder}/icao_aircraft_types.js`)
      .then(response => response.json())
      .then(function(typeLookupData) {
        _aircraft_type_cache = typeLookupData;
      });
    if (!onMobile) {
      fetch(`${databaseFolder}/files.js`)
        .then(response => response.json())
        .then(function(data) {
          for (var i in data) {
    ...
    fetch(`${databaseFolder}/airport-coords.js`)
      .then(response => response.json())
      .then(function(data) {
        ...
        try {
          ...
          if (airport) {
            ...
            if (coords) {
              ...
              if (!search.get('zoom')) OLMap.getView().setZoom(11);
            }
          }
        } catch (error) {
          console.log(error);
...
function init_page() {
  ...
  $('#clock_div').textContent = new Date().toLocaleString();
  ...
  else $('#sidebar_container').width('25%');
...
// check if the top will be less than zero, which will be overlapping/off the screen, and set the top correctly.
  ...
  $('#trace_back_1d').click(function() {
    shiftTrace(-1);
  });
  $('#trace_jump_1d').click(function() {
    shiftTrace(1);
  });
  ...
  if (onMobile) {
    $('#large_mode_button').style.width = 'calc( 45px * var(--SCALE))';
    $('#large_mode_button').style.height = 'calc( 45px * var(--SCALE))';
    if (localStorage['largeMode'] == undefined && largeMode == 1) largeMode = 2;
    globeTableLimit = 40;
  }
...
function push_history() {
  $('#loader_progress').setAttribute('max', nHistoryItems * 2);
...
function push_history_item(i) {
  $.when(deferHistory[i])
    .then(function(json) {
      ...
      $('#loader_progress').setAttribute('value', HistoryItemsReturned);
...
function parse_history() {
  ...
  for (i in deferHistory) deferHistory[i] = null;
  ...
  if (PositionHistoryBuffer.length > 0) {
    ...
    while ((data = PositionHistoryBuffer.pop())) {
      ...
      if (PositionHistoryBuffer.length < 10) {
        ...
        if (now - new Date().getTime() / 1000 > 600) historyOutdated = true;
      } else {
      ...
      if (data.uat_978 != 'true') {
        for (var i = 0; i < PlanesOrdered.length; ++i) {
          ...
          if (plane.dataSource == 'uat') plane.updateTrack(uat_now, uat_last);
          else plane.updateTrack(now, last);
        }
      }
  ...
  if (nHistoryItems) console.timeEnd('Loaded aircraft tracks from History');
  ...
  if (globeIndex) geoFindMe();
  else {
    initSitePos();
  }

  if (!icaoFilter && globeIndex) toggleTableInView(true);

  changeZoom('init');
  changeCenter('init');

  if (globeIndex) setInterval(checkMovement, 100);
  else setInterval(checkMovement, 30);
  ...
  if (localStorage['sidebar_visible'] == 'false') toggleSidebarVisibility();
...
function initialize_map() {
    ...
    if (lyr.get('type') === 'base') {
      ...
      lyr.on('change:visible', function(evt) {
        if (evt.target.getVisible()) {
          MapType_tar1090 = localStorage['MapType_tar1090'] = evt.target.get(
            'name'
          );
        }
      });
  ...
  OLMap = new ol.Map({
    ...
    controls: [
      new ol.control.Zoom({ delta: 1, duration: 0 }),
  ...
  if (
    ...
    (MapDim && localStorage['MapDim'] == null)
  ) {
    toggleMapDim(true);
  }
      ...
      switch (e.key) {
        ...
        case 'H':
          if (!hideButtons) {
            ...
            $('#large_mode_control').style.display = 'block';
            $('#header_top').style.display = 'block';
            $('#header_side').style.display = 'block';
            $('#splitter').style.display = 'block';
            $('#jumpSearch').style.display = 'block';
            $('#filterButton').style.display = 'block';
            $('.ol-control').style.display = 'block';
  ...
  if (!globeIndex) {
    var request = fetch('upintheair.json', {
      cache: 'no-cache'
    });
...
function refreshSelected() {
  ...
  if (selected.flight && selected.flight.trim()) {
    $('#selected_callsign').textContent = selected.flight;
  } else {
    $('#selected_callsign').textContent = 'n/a';
  }
  if (flightawareLinks) {
  ...
  if (selected.registration) {
    if (flightawareLinks) {
      $('#selected_registration').html(
        getFlightAwareIdentLink(selected.registration, selected.registration)
      );
    } else {
      $('#selected_registration').textContent = selected.registration;
    }
  } else {
    $('#selected_registration').textContent = 'n/a';
  }

  if (selected.icaoType) {
    $('#selected_icaotype').textContent =
      selected.typeDescription + ' - ' + selected.icaoType;
  } else {
    $('#selected_icaotype').textContent = 'n/a';
  }

  if (showPictures && selected.icaoType) {
    var new_html =
      "<img width='150px' src='aircraft_sil/" + selected.icaoType + ".png' />";
    if (new_html != selectedPhotoCache) {
      $('#selected_photo').html(new_html);
      selectedPhotoCache = new_html;
    }
  } else {
    $('#selected_photo').textContent = '';
  }
  ...
  $('#selected_altitude1').textContent = format_altitude_long(
    selected.altitude,
    selected.vert_rate,
    DisplayUnits
  );

  $('#selected_altitude2').textContent = format_altitude_long(
    selected.altitude,
    selected.vert_rate,
    DisplayUnits
  );

  $('#selected_onground').textContent = format_onground(selected.altitude);

  if (selected.squawk === null || selected.squawk === '0000') {
    $('#selected_squawk1').textContent = 'n/a';
    $('#selected_squawk2').textContent = 'n/a';
  } else {
    $('#selected_squawk1').textContent = selected.squawk;
    $('#selected_squawk2').textContent = selected.squawk;
  }

  $('#selected_speed1').textContent = format_speed_long(
    selected.gs,
    DisplayUnits
  );
  $('#selected_speed2').textContent = format_speed_long(
    selected.gs,
    DisplayUnits
  );
  $('#selected_ias').textContent = format_speed_long(
    selected.ias,
    DisplayUnits
  );
  $('#selected_tas').textContent = format_speed_long(
    selected.tas,
    DisplayUnits
  );
  if (selected.geom_rate != null) {
    $('#selected_vert_rate').textContent = format_vert_rate_long(
      selected.geom_rate,
      DisplayUnits
    );
  } else {
  ...
  $('#selected_baro_rate').textContent = format_vert_rate_long(
    selected.baro_rate,
    DisplayUnits
  );

  $('#selected_geom_rate').textContent = format_vert_rate_long(
    selected.geom_rate,
    DisplayUnits
  );
  ...
  if (adsbexchange) {
    ...
    $('#selected_icao').textContent = selected.icao.toUpperCase();
  }
  $('#selected_pf_info').textContent = selected.pfRoute ? selected.pfRoute : '';
  //+" "+ (selected.pfFlightno ? selected.pfFlightno : "")
  $('#airframes_post_icao').setAttribute('value', selected.icao);
  $('#selected_track1').textContent = format_track_long(selected.track);
  $('#selected_track2').textContent = format_track_brief(selected.track);

  if (selected.seen != null && selected.seen < 1000000) {
    $('#selected_seen').textContent = format_duration(selected.seen);
  } else {
    $('#selected_seen').textContent = 'n/a';
  }
  if (selected.seen_pos != null && selected.seen_pos < 1000000) {
    $('#selected_seen_pos').textContent = format_duration(selected.seen_pos);
  } else {
    $('#selected_seen_pos').textContent = 'n/a';
  }

  $('#selected_country').textContent = selected.icaorange.country.replace(
    'special use',
    'special'
  );
  ...
  if (ShowFlags && selected.icaorange.flag_image !== null) {
    ...
    $('#selected_flag img').setAttribute(
      'src',
      FlagPath + selected.icaorange.flag_image
    );
    $('#selected_flag img').setAttribute('title', selected.icaorange.country);
  } else {
    $('#selected_flag').addClass('hidden');
  }

  if (selected.position === null) {
    $('#selected_position').textContent = 'n/a';
    $('#selected_follow').addClass('hidden');
  } else {
    if (selected.seen_pos > -1) {
      $('#selected_position').textContent = format_latlng(selected.position);
    } else {
      $('#selected_position').textContent = format_latlng(selected.position);
    }
    ...
    if (FollowSelected) {
      $('#selected_follow').style.fontWeight = 'bold';
      if (selected.position)
        OLMap.getView().setCenter(ol.proj.fromLonLat(selected.position));
    } else {
      $('#selected_follow').style.fontWeight = 'normal';
    }
  }
  $('#selected_source').textContent = format_data_source(
    selected.getDataSource()
  );
  $('#selected_category').textContent = selected.category
    ? selected.category
    : 'n/a';
  $('#selected_sitedist1').textContent = format_distance_long(
    selected.sitedist,
    DisplayUnits
  );

  $('#selected_sitedist2').textContent = format_distance_long(
    selected.sitedist,
    DisplayUnits
  );

  $('#selected_rssi1').textContent =
    selected.rssi != null ? selected.rssi.toFixed(1) : 'n/a';

  $('#selected_message_count').textContent = selected.messages;
  $('#selected_message_rate').textContent =
  ...
  $('#selected_altitude_geom').textContent = format_altitude_long(
    selected.alt_geom,
    selected.geom_rate,
    DisplayUnits
  );

  $('#selected_mag_heading').textContent = format_track_brief(
    selected.mag_heading
  );
  $('#selected_true_heading').textContent = format_track_brief(
    selected.true_heading
  );
  $('#selected_ias').textContent = format_speed_long(
    selected.ias,
    DisplayUnits
  ...
  if (selected.mach == null) {
    $('#selected_mach').textContent = 'n/a';
  } else {
    $('#selected_mach').textContent = selected.mach.toFixed(3);
  }
  if (selected.roll == null) {
    $('#selected_roll').textContent = 'n/a';
  } else {
    $('#selected_roll').textContent = selected.roll.toFixed(1);
  }
  if (selected.track_rate == null) {
    $('#selected_trackrate').textContent = 'n/a';
  } else {
    $('#selected_trackrate').textContent = selected.track_rate.toFixed(2);
  }
  $('#selected_geom_rate').textContent = format_vert_rate_long(
    selected.geom_rate,
    DisplayUnits
  ...
  if (selected.nav_qnh == null) {
    $('#selected_nav_qnh').textContent = 'n/a';
  } else {
    $('#selected_nav_qnh').textContent = selected.nav_qnh.toFixed(1) + ' hPa';
  }
  $('#selected_nav_altitude').textContent = format_altitude_long(
    selected.nav_altitude,
    0,
    DisplayUnits
  );

  $('#selected_nav_heading').textContent = format_track_long(
    selected.nav_heading
  );
  if (selected.nav_modes == null) {
    $('#selected_nav_modes').textContent = 'n/a';
  } else {
    $('#selected_nav_modes').textContent = selected.nav_modes.join();
  }
  if (selected.nic_baro == null) {
    $('#selected_nic_baro').textContent = 'n/a';
  } else {
    if (selected.nic_baro == 1) {
      $('#selected_nic_baro').textContent = 'cross-checked';
    } else {
      $('#selected_nic_baro').textContent = 'not cross-checked';
    }
  }

  $('#selected_nac_p').textContent = format_nac_p(selected.nac_p);
  $('#selected_nac_v').textContent = format_nac_v(selected.nac_v);
  if (selected.rc == null) {
    $('#selected_rc').textContent = 'n/a';
  } else if (selected.rc == 0) {
    $('#selected_rc').textContent = 'unknown';
  } else {
    $('#selected_rc').textContent = format_distance_short(
      selected.rc,
      DisplayUnits
    );
  }
  ...
  if (selected.version == null) {
    $('#selected_version').textContent = 'none';
  } else if (selected.version == 0) {
    $('#selected_version').textContent = 'v0 (DO-260)';
  } else if (selected.version == 1) {
    $('#selected_version').textContent = 'v1 (DO-260A)';
  } else if (selected.version == 2) {
    $('#selected_version').textContent = 'v2 (DO-260B)';
  } else {
    $('#selected_version').textContent = 'v' + selected.version;
  }
...
function refreshHighlighted() {
  ...
  $('#highlighted_infoblock').style.display = 'block';
  ...
  else infoBox.css('left', markerPosition[0] - 200);
  if (markerPosition[1] + 250 < mapSize[1])
    infoBox.css('top', markerPosition[1] + 50);
  else infoBox.css('top', markerPosition[1] - 250);

  $('#highlighted_callsign').textContent = highlighted.name;

  if (highlighted.icaoType !== null) {
    $('#highlighted_icaotype').textContent = highlighted.icaoType;
  } else {
    $('#highlighted_icaotype').textContent = 'n/a';
  }

  $('#highlighted_source').textContent = format_data_source(
    highlighted.getDataSource()
  );

  if (highlighted.registration !== null) {
    $('#highlighted_registration').textContent = highlighted.registration;
  } else {
    $('#highlighted_registration').textContent = 'n/a';
  }

  $('#highlighted_speed').textContent = format_speed_long(
    highlighted.speed,
    DisplayUnits
  );

  $('#highlighted_altitude').textContent = format_altitude_long(
    highlighted.altitude,
    highlighted.vert_rate,
    DisplayUnits
  );

  $('#highlighted_pf_route').textContent = highlighted.pfRoute
    ? highlighted.pfRoute
    : highlighted.icao.toUpperCase();

  $('#highlighted_rssi').textContent =
...
function refreshClock(now_date) {
  ...
  var hms =
    ...
    now_date
      ...
      .toString()
      .padStart(2, '0');
  $('#clock_div').textContent = hms + '   ' + now_date.toDateString();
}
...
function refreshTableInfo() {
  ...
  $('#dump1090_total_history').textContent = TrackedHistorySize;

  if (MessageRate !== null) {
    $('#dump1090_message_rate').textContent = MessageRate.toFixed(1);
  } else {
    $('#dump1090_message_rate').textContent = 'n/a';
  }
  ...
  for (var i = 0; i < PlanesOrdered.length; ++i) {
    ...
    if (!tableplane.isFiltered() && (tableplane.seen < 58 || noVanish)) {
      ...
      if (!tableInView) tableplane.showInTable = true;
    }

    if (
      !sidebarVisible ||
      (nTablePlanes > globeTableLimit && mapIsVisible && globeIndex)
    ) {
      tableplane.showInTable = false;
      continue;
    }
    ...
    if (tableplane.showInTable) {
      ...
      if (tableplane.selected && !SelectedAllPlanes) classes += ' selected';
      ...
      if (flightawareLinks) {
        ...
        updateCell(
          tableplane,
          3,
          tableplane.registration ? tableplane.registration : ''
        );
      }
      updateCell(
        tableplane,
      ...
      updateCell(
        tableplane,
        7,
        format_speed_brief(tableplane.gs, DisplayUnits)
      );
      updateCell(
        tableplane,
        8,
        format_vert_rate_brief(tableplane.vert_rate, DisplayUnits)
      );
      updateCell(
        tableplane,
        9,
        format_distance_brief(tableplane.sitedist, DisplayUnits)
      );
      ...
      updateCell(
        tableplane,
        13,
        tableplane.rssi != null ? tableplane.rssi.toFixed(1) : ''
      );
      updateCell(
        tableplane,
        14,
      ...
      updateCell(
        tableplane,
        16,
        format_data_source(tableplane.getDataSource())
      );
  ...
  if (show_squawk_warning_cache != show_squawk_warning && show_squawk_warning) {
    $('#SpecialSquawkWarning').style.display = 'block';
  ...
  if (
    ...
    $('#SpecialSquawkWarning').style.display = 'none';
    show_squawk_warning_cache = show_squawk_warning;
  }

  if (!globeIndex) $('#dump1090_total_ac').textContent = TrackedAircraft;
  else $('#dump1090_total_ac').textContent = globeTrackedAircraft;
  $('#dump1090_total_ac_positions').textContent = TrackedAircraftPositions;
...
function compareBeta(xa, ya) {
  if (xa === ya) return 0;
  if (sortAscending && xa < ya) return -1;
  if (
    !sortAscending &&
    xa
      .replace(/ /g, '')
...
function sortByFlight() {
  sortBy('flight', compareBeta, function(x) {
    return x.flight ? x.flight : x.registration;
  });
}
...
function sortByBaseMarkerKey() {
  sortBy('base_marker_key', compareAlpha, function(x) {
    return x.baseMarkerKey;
  });
}
  ...
  if (!options.noFetch && globeIndex && hex) {
    ...
    if (req1) {
      req1.then(function(data) {
        ...
        if (this.follow) FollowSelected = true;
      });
    }
    req2.then(function(data) {
...
function deselectAllPlanes() {
  if (showTrace) return;
  if (!multiSelect && SelectedPlane) toggleIsolation(false, 'off');
...
function toggleFollowSelected() {
  ...
  if (FollowSelected && (!SelectedPlane || !SelectedPlane.position))
    FollowSelected = false;
  if (!SelectedPlane && FollowSelected) FollowSelected = false;
...
function toggleSidebarVisibility(e) {
  ...
  localStorage['sidebar_visible'] = sidebarVisible = $('#sidebar_container').is(
    ':visible'
  );
  updateMapSize();
}
...
function expandSidebar(e) {
  ...
  $('#splitter').hide();
  $('#sudo_buttons').hide();
  $('#show_map_button').style.display = 'block';
...
function showMap() {
  $('#sidebar_container')
    .width(localStorage['sidebar_width'])
    .css('margin-left', '0');
  $('#map_container').style.display = 'block';
  mapIsVisible = true;
  $('#toggle_sidebar_control').style.display = 'block';
  $('#splitter').style.display = 'block';
  $('#sudo_buttons').style.display = 'block';
  $('#show_map_button').hide();
  setColumnVisibility();
  clearTimeout(refreshId);
  fetchData();
...
function setSelectedInfoBlockVisibility() {
  if (SelectedPlane) {
    $('#selected_infoblock').style.display = 'block';
    if (!mapIsVisible) $('#sidebar_container').style.marginLeft = '140pt';
    //$('#sidebar_canvas').css('margin-bottom', $('#selected_infoblock').height() + 'px');
    //
    $('#large_mode_control').style.left = `${190 * globalScale}px`;
  } else {
    $('#selected_infoblock').hide();
    if (!mapIsVisible) $('#sidebar_container').style.marginLeft = '0';
    //$('#sidebar_canvas').css('margin-bottom', 0);

    $('#large_mode_control').style.left = `${5 * globalScale}px`;
  }
}
...
function initializeUnitsSelector() {
  ...
  $('.altitudeUnit').textContent = get_unit_label('altitude', DisplayUnits);
  $('.speedUnit').textContent = get_unit_label('speed', DisplayUnits);
  $('.distanceUnit').textContent = get_unit_label('distance', DisplayUnits);
  $('.verticalRateUnit').textContent = get_unit_label(
    'verticalRate',
    DisplayUnits
  );
}
...
function toggleIsolation(on, off) {
  ...
  if (on) onlySelected = true;
  if (off) onlySelected = false;
...
function toggleLastLeg() {
  ...
  if (SelectedPlane) SelectedPlane.processTrace();
}
...
function toggleAltitudeChart(switchToggle) {
  ...
  if (altitudeChartDisplay === 'show') {
    ...
    $('#altitude_chart').style.display = 'block';
  } else {
...
function toggleTableInView(switchOn) {
  ...
  if (tableInView) {
    $('#with_positions').textContent = 'On Screen:';
  } else {
    $('#with_positions').textContent = 'With Position:';
  }
...
function toggleExtendedLabels() {
  if (isNaN(extendedLabels)) extendedLabels = 0;
...
function toggleMultiSelect(on, off) {
  ...
  if (on) multiSelect = true;
  if (off) multiSelect = false;

  if (!multiSelect) {
    if (!SelectedPlane) toggleIsolation(false, 'off');
    var plane = SelectedPlane;
    SelectedPlane = null;
    deselectAllPlanes();
    if (plane) selectPlaneByHex(plane.icao);
  }
...
function onSearch(e) {
  ...
  $('#search_input').val('');
  $('#search_input').blur();
  if (searchTerm) findPlanes(searchTerm, true, true, true, true);
  return false;
}
...
function updateCallsignFilter(e) {
  ...
  PlaneFilter.callsign = $('#callsign_filter')
    .val()
    .trim()
    .toUpperCase();
...
function updateDescriptionFilter(e) {
  ...
  $('#description_filter').blur();
  var description = $('#description_filter')
    .val()
    .trim();
...
function updatePlaneFilter() {
  var minAltitude = parseFloat(
    $('#altitude_filter_min')
      .val()
      .trim()
  );
  var maxAltitude = parseFloat(
    $('#altitude_filter_max')
      .val()
      .trim()
  );
...
function getFlightAwarePhotoLink(registration) {
  if (registration !== null && registration !== '') {
    return (
      ...
      registration.replace(/[^0-9a-z]/gi, '') +
      '">See Photos</a>'
    );
  }
...
function updatePiAwareOrFlightFeeder() {
  if (isFlightFeeder) {
    $('.piAwareLogo').hide();
    $('.flightfeederLogo').style.display = 'block';
    PageName = 'FlightFeeder SkyAware';
  } else {
    $('.flightfeederLogo').hide();
    $('.piAwareLogo').style.display = 'block';
...
function changeCenter(init) {
  ...
  if (!onlySelected) refreshTableInfo();
...
function changeZoom(init) {
  ...
  if (!init && Math.abs(ZoomLvl - ZoomLvlCache) < 0.4) return;
  ...
  if (globalScale <= 1.15)
    scaleFactor = Math.max(
      ...
      Math.min(
        markerMaxSize,
        markerScaleFactor * 0.09 * Math.pow(1.35, ZoomLvl)
      )
    );
  else {
    scaleFactor = Math.max(
      markerMinSize * Math.pow(1.3, globalScale) * globalScale,
      Math.min(
        markerMaxSize,
        markerScaleFactor * 0.09 * Math.pow(1.35, ZoomLvl)
      )
...
function processURLParams() {
  try {
    ...
    if (icaoFilter) icaoFilter = icaoFilter.toLowerCase().split(',');
    ...
    if (icao != null) {
      if (!search.has('noIsolation')) toggleIsolation('on', false);
    ...
    if (search.has('hideButtons')) hideButtons = true;
  } catch (error) {
    console.log(error);
  }
}
...
function findPlanes(query, byIcao, byCallsign, byReg, byType) {
  ...
  if (byReg && regCache[query.toUpperCase()]) {
    selectPlaneByHex(regCache[query.toUpperCase()].toLowerCase(), {
      follow: true
    });
    return;
  }
  for (var i in PlanesOrdered) {
    ...
    if (
      ...
      if (plane.seen < 70 || noVanish) results.push(plane);
    }
  }
  if (results.length > 1) {
    ...
    if (
      globeIndex &&
      query.length == 6 &&
      query.toLowerCase().match(/[a-f,0-9]{6}/)
    ) {
...
function inView(tableplane, currExtent) {
  if (tableplane.position == null) return false;
  if (tableplane.isFiltered()) return false;
  ...
  const proj = tableplane.position
    ? ol.proj.fromLonLat(tableplane.position)
    : null;

  if (pos && currExtent[2] - currExtent[0] > 40075016) {
    // all longtitudes in view, only check latitude
    inView = pos[1] > bottomLeft[1] && pos[1] < topRight[1];
...
function updateAddressBar() {
  var posString =
    ...
    ZoomLvl.toFixed(1);
  var string;
  if (true || !globeIndex) posString = '';
  else if (SelectedPlane) posString = '&' + posString;
  else posString = '?' + posString;

  if (SelectedPlane)
    string = pathName + '?icao=' + SelectedPlane.icao + posString;
  else string = pathName + posString;
...
function refreshInt() {
  ...
  if (!globeIndex) return refresh;

  if (!mapIsVisible) refresh *= 2;

  if (onMobile) refresh *= 1.5;

  if (lastRequestFiles >= 4) return 1.6 * refresh;
...
function toggleLargeMode() {
  largeMode++;
  if (!(largeMode >= 1 && largeMode <= 4)) largeMode = 1;
  ...
  labelFont =
    'bold ' + 12 * globalScale + 'px Tahoma, Verdana, Helvetica, sans-serif';
...
function shiftTrace(offset) {
  ...
  $('#trace_date').textContent = 'UTC day:' + traceDateString;
